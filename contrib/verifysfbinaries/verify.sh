...
###   This script attempts to download the signature file SHA256SUMS.asc from goldbit.org
...
WORKINGDIR="/tmp/goldbit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://goldbit.org/bin/"
VERSIONPREFIX="goldbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'goldbit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Goldbit Core binary release signing key installed?"
   fi
